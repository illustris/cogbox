...
// Darwin rejects SOCK_CLOEXEC in socket's type argument. Keep the descriptor
// private across exec using fcntl there; Linux sets it atomically at creation.
pub fn streamSocket(domain: c_int) !std.posix.fd_t {
	const flags = if (darwin) 0 else std.posix.SOCK.CLOEXEC;
	const fd = std.c.socket(@intCast(domain), std.posix.SOCK.STREAM | flags, 0);
	if (fd < 0) return error.SocketFailed;
	errdefer _ = std.c.close(fd);
	if (darwin and std.c.fcntl(fd, std.posix.F.SETFD, @as(c_int, std.posix.FD_CLOEXEC)) < 0)

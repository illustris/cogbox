...
- the conf is a JSON list of specs `{host, style, cred_file, token_path?, cred_format?, cookie_name?, account_id_path?, refresh?, stub_token?, rules_tag?, on_guest_credential?}`;
...
- **Precedence over a guest credential** (`on_guest_credential`). Chosen **per bind** with `cogbox secret add --on-guest-credential replace|keep`, stored in the secret's meta and read from there by the render for every `bearer` / `cookie` / `basic` element -- a plugin spec naming that secret included, so a manifest can never choose it. The render emits the conf key **only for `keep`**, so a `replace` bind's element is byte-identical to what it was before the key existed, and the addon reads a missing key as `replace`. `replace` (the default, today's behaviour): the proxy always **sets** the credential -- `Authorization: Bearer <value>`, `Authorization: Basic base64(user:password)`, or the named cookie via `set_cookie` with every other cookie verbatim. `keep`: inject **only when the style's slot is empty**; otherwise the guest's credential is forwarded untouched (`apply_injection` is not called) and the upstream judges it. Per style, the slot is: `bearer` and `basic` -- the whole `Authorization` request header, where **any** non-empty value of any scheme counts as "the sandbox sent a credential" (no scheme parsing; a `PRIVATE-TOKEN` header with no `Authorization` counts as none); `cookie` -- the single named cookie (present with a non-empty value = keep; absent or empty = inject; other cookies are irrelevant and always preserved). A spec carrying `stub_token` keeps the stricter stub gate regardless (the stub still counts as "empty"); the `anthropic-oauth` / `gitlab-oauth` kinds never carry the key. `keep` can only ever inject **less** than `replace`: a compromised guest can suppress injection for its own requests, never redirect the operator's credential. The stale-conf fail-closed path (`conf_unavailable_for`) asks exactly the same question off the remembered spec shape, so under `keep` only a credential-less request is denied while the conf is unreadable.
- the audience gate (below) compares hosts **case-insensitively with one trailing dot stripped**, exactly as the addon keys its specs -- a folding of spelling only, never of identity.
...
```sh
cogbox secret add api-bearer --from-file ~/.secrets/api.token --audience api.example.com            # a plugin spec names it
cogbox secret add api-bearer --from-file ~/.secrets/api.token --audience api.example.com --inject   # no plugin spec needed
cogbox secret ls
...
| Flag | Meaning |
|---|---|
| `--from-file F` / `--from-stdin` | the value source (never argv) |
| `--audience HOST` | the one exact host the secret may be injected to -- the **bare** host, no port; the render folds case and a trailing dot when it compares. With `--inject` it must itself pass the L7 host grammar (LDH labels or an IP literal; no wildcard, port, path, whitespace or trailing dot -- the check a plugin spec's `host` passes at `plugin add`), because the render turns it into an `l7-rules` / `l7-inject-hosts` **line**: the verb refuses anything else (exit 65) and the render skips a hand-edited meta carrying one with a warning |
| `--kind bearer\|cookie\|basic` | the wire style (table below); the platform kinds `anthropic-oauth`, `gitlab-oauth` and `gitlab-authproxy` are bound by a control plane, never with `--inject` |
| `--inject` | make this a **free-form injecting bind**: the render seeds an inject spec `{host: audience, style: kind, secret: name}` for it, so no plugin spec is needed (see below). Requires `--audience`; `bearer` / `cookie` / `basic` only |
| `--cookie-name N` | with `--kind cookie`: the cookie the proxy replaces (required together with `--inject`; a plugin spec supplies its own `cookieName`) |
| `--port P` | with `--inject`: the host is served on `P` rather than 80/443, so that port is funnelled too (see [non-standard ports](#non-standard-ports)) |
| `--on-guest-credential replace\|keep` | what to do when the sandbox's request already carries a credential (see [How it works](#how-it-works)); default `replace`. Meaningful on a plugin-spec bind too: the choice rides through the plugin's spec |
| `-n INSTANCE` | re-render that instance's runtime files afterwards, so the bind applies to a running VM without a restart |

Exit codes: `0` bound; `64` usage -- including an **unknown flag**, which fires before the value is read, so an older binary handed `--inject` binds nothing at all (a control plane keys "this agent predates operator inject binds" off exactly that pair: exit 64 plus `cogbox secret: error: unknown flag '--inject'` on stderr); `65` an invalid value (`--inject` without `--audience` or on a platform kind, an `--inject` audience that is not an exact bare host, a cookie bind without `--cookie-name`, `--cookie-name` on a non-cookie kind, a bad port or precedence), also before the value is read; `73` the store could not be written.

`cogbox secret ls --json` emits a JSON array of `{name, kind, audience, tier, bound, bound_at, inject, cookie_name, port, on_guest_credential}` (the **value is never included**). A control plane (e.g. cogworx) reads it to show each plugin-declared inject request as bound vs unbound -- correctly reflecting the host-side store even when an operator bound the secret directly with `cogbox secret add` rather than through the UI. `audience` is `null` when unset (not injectable), `bound` is `false` when the named secret has no value file. The four operator-inject keys are **always** present on a binary that knows them (a control plane reads their absence as "agent predates the feature"): `inject` is `false` for every pre-feature or plugin-spec bind, `cookie_name` and `port` are `null` when unset, `on_guest_credential` is `replace` or `keep`. A store that was never created prints `[]`.

The value is read from a file or stdin (never argv, which leaks to the process table) and stored at `~/.config/cogbox/secrets/<name>` (mode `0600`) alongside a `<name>.meta` sidecar recording `audience`, `kind`, `tier`, `bound_at`, `inject`, `cookie_name`, `port` and `on_guest_credential` (a sidecar written before those keys existed reads as an inert `replace` bind; an older binary ignores the new keys -- both directions fail closed). Where the deployment runs the proxy on a dedicated uid (`COGBOX_PROXY_RUNAS`), a bind with an `--audience` lands `0640` with the proxy's group instead, staged before the file is nameable so it is injectable from the first instant rather than from the render that follows -- see [Credential access under a dropped proxy uid](#credential-access-under-a-dropped-proxy-uid). The stored value is a single line -- a bare bearer token, a `user:password` pair, or a cookie value -- interpreted according to `--kind`.
...
Injection needs two things: an inject **spec** (which host + style + secret name, and -- on a non-standard port -- the `port`) and the **bound secret**. A plugin's `cogboxPlugins.inject` writes the spec for you; without a plugin, `--inject` makes **the bind itself the spec** -- there is no `cogbox inject add` verb and nothing to hand-edit:

```sh
# Bind the credential host-side (raw user:password; base64 is done at injection time)
# and let the render seed the inject spec for it. The --audience is the BARE host --
# no :9200 -- and the :9200 lives in --port. `-n` re-renders a running instance.
echo -n "elastic:mypassword" | cogbox secret add es-creds --from-stdin \
    --audience es.internal.example.com --kind basic --inject --port 9200 -n <name>
```

What `--inject` does, precisely. On every render the enforcer seeds an inject spec for each bound `bearer` / `cookie` / `basic` secret whose meta says `inject: true` (`seedOperatorInjectSpecs`, run after the claude and git seeds), and the existing consumers do the rest: `l7-rules` gains `allow <host> terminate` for the audience unless an L7 rule already names it, `netfilter-rules` funnels the `--port`, `l7-inject-conf.json` gains the element and `l7-inject-hosts` the host. Five properties to know:

- **The audience is one exact bare host, or nothing is seeded.** `l7-rules` and `l7-inject-hosts` are whitespace-tokenized line formats, so the seed only ever writes an audience that passes the L7 host grammar (`filter.isValidHostName`: LDH labels or an IP literal; no wildcard, port, path, whitespace or trailing dot -- the gate a plugin spec's `host` passes at `plugin add`). A meta whose audience would otherwise smuggle rule tokens (`insecure`, `passthrough`) or whole lines (`tag=git-grants`, `mode passthrough`) into the policy is skipped with `cogbox: warning: render: secret '<name>' not injected: its audience is not an exact bare host`; the verb refuses the same audience with `--inject` up front (exit 65).
- **Whole-host allow on bind (plugin parity).** That `allow <host> terminate` is a whole-host terminate-allow, exactly what a plugin's inject spec produces at `plugin add` -- so an injecting bind widens egress by one exact host, the same as installing a plugin that declares one, and a control plane should gate `--inject` on the principals it lets install plugins. A `deny` rule naming the host suppresses the union while the host still routes to the addon, which denies: fail closed. (The `gitlab-oauth` seed is different on purpose -- it refuses to seed at all unless a rule names the host.)
- **One spec per host.** A plugin spec, the claude seed, the git seed or an alphabetically-earlier injecting bind already targeting the audience (compared case-insensitively, trailing dot stripped) wins; the bind is skipped with a `cogbox: warning: render: secret '<name>' not injected: host <h> already has an inject spec` on stderr. A plugin's spec is authoritative for its host -- bind the plugin's named secret instead of a second one. An instance-store secret shadows a global one of the same name, as everywhere else. The launcher's **harness specs** are a fourth writer this rule cannot see: `gen_inject_conf` merges them into `l7-inject-conf.json` *after* the boot render, from the launcher's shell state rather than any config or store (see [the two-writer contract](#where-the-two-writer-contract-actually-applies)), so the seed never finds them and a bind on a harness host is seeded regardless. They win the host anyway -- at the addon by last-write-by-host, and the host is never HTTP-routed (stripped from `l7-inject-hosts` at boot, withheld by every live render) -- so an injecting bind on a harness host (`api.anthropic.com` with no `claude-oauth` bound, `chatgpt.com`, ...) on a box with host login is inert on the wire, with `cogbox-launch: warning: withholding plain-HTTP inject-routing for harness host(s) ...` (boot) / `cogbox: warning: render: host <h>: plain-HTTP inject-routing withheld ...` (live) on stderr.
- **Cap guards.** `cogbox l7 replace` budgets the terminate-allow union for config specs only (it runs before any seed), so a seeded union line is exactly what could push the rendered `l7-rules` past `max_l7_rules` (128 rule lines: the enforcer compiles the first 128 and drops the rest in silence while the addon reading the same document has no cap). A bind whose union line would cross that cap, or whose host would push `l7-inject-hosts` past its 64-line reader, is skipped with a warning instead of rendered over the cap.
- **A render-time overlay, not config.** Nothing is written to `config.json`, `cogbox l7 list` does not show the seeded spec, and `cogbox secret rm` withdraws the element, the hosts line, the allow line and the funnel at the next render -- unbind is symmetric for free. A bind without `--inject` (every bind before the flag existed, and every plugin "bind now") stays exactly as inert as before: only a plugin spec naming it injects.

The guest then sends requests unauthenticated or with a placeholder, and the host proxy rewrites the `Authorization` header before the request leaves the host (or keeps the guest's own, if the bind chose `--on-guest-credential keep`). The spec host, the `--audience`, and any explicit `cogbox l7 add allow` all use the **bare** host (the proxy matches the request `Host` with its port stripped); the `:9200` lives only in the spec's `port` / the bind's `--port` -- see [non-standard ports](#non-standard-ports). Omit it and the bind still injects on `:80`/`:443` but a `:9200` request never reaches the proxy and stays unauthenticated.
...
**Some binds are auto-seeded.** Three secret shapes get their spec from the renderer itself rather than from a plugin, so no `config.json` entry is needed, and none is written (the seed is a render-time overlay, so revoking is just unbinding):
...
- any `bearer` / `cookie` / `basic` secret bound with `--inject` -- seeds `{host: audience, style: kind, secret: name[, cookieName][, port]}`, one spec per host with the seeds above and any plugin spec winning, whole-host allow-on-bind like a plugin spec, under the cap guards -- see [The secret store](#the-secret-store).

...
- **Inbound header allowlist, not a denylist.** `Authorization`, `Private-Token`, `Cookie`, hop-by-hop headers and every `X-Cogbox-*` are dropped; `X-Forwarded-For` is never synthesized. Operational consequence, because the addon retargets **every** allowed flow for a host in `l7-auth-hosts`: while a grant is active for a host, the grant's policy governs all of that host's traffic from the guest — a credential the guest holds itself (a netrc, a PAT in a header), a credential another plugin's inject spec would have stamped, and an operator bind made with `cogbox secret add --inject` on that host are all superseded, not merged. The operator case is the one that now legitimately puts an inject spec and an auth entry on the same host: the render seeds the bind's spec beside the auth entry, the addon retargets before it ever consults the spec, and logs a one-shot `CONFLICT host=… (the grant supersedes the bound credential)` notice per conf generation — informational, no longer "control-plane bug" — while cogworx labels the bind *superseded by Git access* on the Network tab. They apply again only once no grant names the host.
